...
int main( int argc, char** argv )
{
    ...
    std::ofstream out( "/home/tfm3/workspace/RegistradorSVDRansac/miSalidaRegistro.txt" );
    out << std::setprecision(6) << std::fixed;
	std::ifstream infile("/home/tfm3/workspace/RegistradorSVDRansac/miEntrada.txt");
	//std::string line;
	...
	std::ifstream infile2("/home/tfm3/workspace/RegistradorSVDRansac/miEntradaModificada.txt");
	//std::string line;
    //infile >> std::setprecision(6) >> std::fixed;
	//MatrixXd A (3000,3);
	std::cout <<"antes de declarar la variable"<<std::endl;
...
while (iterations < maxIterations){ // Comenzamos el bucle RANSANC
	...
	Registrador miRegistradorSVD;
	MatrixXd ret_R,ret_t,second_xyz_aligned;
	//miRegistradorHorn.align(B.transpose(),A.transpose(),ret_R,ret_t);
	miRegistradorSVD.rigid_transform_3D(mInliers,mData,ret_R,ret_t); // Estimamos matriz rotacion traslacion
    std::cout<< "despues de rigid_transform"<<std::endl;
    ...
    miRegistradorSVD.rigid_transform_3D(mFinalInliers,mFinalData,ret_R,ret_t);
